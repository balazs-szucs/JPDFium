...
JPDFIUM_EXPORT int32_t jpdfium_pdfio_try_repair(const uint8_t* input, int64_t inputLen,
    ...
    *pagesRecovered = 0;
    ...
    char errBuf[512] = {0};
    pdfio_file_t* inPdf = pdfioFileOpen(tmpIn, /*password_cb*/ nullptr, /*password_data*/ nullptr,
    ...
    pdfio_file_t* outPdf =
        pdfioFileCreate(tmpOut, /*version*/ "1.7",
                        /*media_box*/ nullptr, /*crop_box*/ nullptr, pdfio_error_cb, errBuf);
    ...
    *outputLen = static_cast<int64_t>(fread(*output, 1, static_cast<size_t>(sz), f));
    ...
    int32_t status = (*pagesRecovered == static_cast<int32_t>(numPages)) ? JPDFIUM_REPAIR_FIXED
